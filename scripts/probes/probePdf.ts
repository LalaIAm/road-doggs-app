...
async function savePDFToFile(pdfBytes: Uint8Array, filePath: string): Promise<void> {
  console.log('💾 Saving PDF to filesystem...');
  ...
  try {
    ...
    console.error('❌ Error saving PDF to file:');
    console.error('   ', error.message);
    console.error('');
    throw error;
  }
...
function verifyPDFFile(filePath: string): void {
  ...
  try {
    ...
    console.log('   ✓ PDF header is valid (%PDF)');
    console.log(`   ✓ File size: ${stats.size} bytes`);
    console.log('');
    ...
    console.error('❌ Error verifying PDF file:');
    console.error('   ', error.message);
    console.error('');
    throw error;
  }
...
function cleanupTestFile(filePath: string): void {
  console.log('🧹 Cleaning up test PDF file...');
  ...
  try {
    ...
    console.log('   ⚠️  Warning: Could not delete test PDF file');
...
async function probePdf() {
  ...
  try {
    console.log('🧪 Probing PDF Generation (pdf-lib)...');
    console.log('');
    ...
    console.log('   ✓ pdf-lib library imported successfully');
    console.log('   ✓ PDF document created');
    console.log('   ✓ PDF saved to filesystem');
    console.log('   ✓ PDF file is valid');
    console.log('   ✓ Library compatibility with Node.js verified');
    console.log('');
    
    // Display file path for user inspection
    console.log('📄 Test PDF created at:');
...
async function main() {
  ...
  if (success) {
    console.log('✅ PDF Generation probe completed successfully!');
    process.exit(0);
  } else {
    console.error('❌ PDF Generation probe failed!');
    process.exit(1);
  }
}
...
// Run probe
main().catch((error) => {
  console.error('💥 Fatal error running PDF Generation probe:', error);
  process.exit(1);
});
